package s4.B233367;  // ここは、かならず、自分の名前に変えよ。
import java.lang.*;
...
public class Frequencer implements FrequencerInterface{
    ...
    private int suffixCompare(int i, int j) {
        ...
        int iLength = mySpace.length - i;
        int jLength = mySpace.length - j;
        int length = (iLength <= jLength) ? iLength : jLength;

        for(int offset = 0; offset < length; ++offset) {
            byte b_i = mySpace[i + offset];
            byte b_j = mySpace[j + offset];
            if(b_i > b_j) return 1;
            if(b_i < b_j) return -1;
        }

        if(iLength > jLength) return 1;
        if(iLength < jLength) return -1;
    ...
    private int targetCompare(int i, int j, int k) {
        ...
        for(int offset = 0; offset < targetLength; ++offset) {
            if(i + offset >= mySpace.length) return -1;
            byte b_i = mySpace[i + offset];
            byte b_j = myTarget[j + offset];
            if(b_i > b_j) return 1;
            if(b_i < b_j) return -1;
        }
